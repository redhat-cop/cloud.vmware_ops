...
if [ -f /tmp/vmware_ops_tests_report.txt ]; then
    rm -f /tmp/vmware_ops_tests_report.txt
fi
...
total=0
{
    ...
    echo "Tests Summary"
    echo "==============="
} >> /tmp/vmware_ops_tests_report.txt
...
for target in $(ansible-test integration --list-target | grep 'vmware_'); do
    ...
    if ansible-test integration $target --skip-tags force-simulator-run; then
        echo -e "Test: $target ${GREEN}Passed${NC}" | tee -a /tmp/vmware_ops_tests_report.txt
    else
        echo -e "Test: $target ${RED}Failed${NC}" | tee -a /tmp/vmware_ops_tests_report.txt
        failed=$((failed + 1))
    fi
done
